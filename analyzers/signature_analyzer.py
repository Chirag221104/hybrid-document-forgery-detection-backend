...
from docx import Document  # Already in your imports
...
class SignatureAnalyzer:
    async def analyze(self, file_path: str, file_info: dict):
        ...
        if file_type == "application/pdf":
            # Your previous PDF code here (from earlier example)
            pass  # Replace with actual PDF detection code
        
        elif file_type in ["application/vnd.openxmlformats-officedocument.wordprocessingml.document", "application/msword"]:
            try:
                # Open DOCX as zip (it's a zipped format)
                with zipfile.ZipFile(file_path, 'r') as docx_zip:
                    # Check for signature directory
                    if any('._xmlsignatures/' in name for name in docx_zip.namelist()):
                        # Extract signature XML (simplified; assumes one signature)
                        ...
                        with docx_zip.open(sig_files[0]) as sig_file:
                            ...
                            # Basic certificate check (load and validate)
